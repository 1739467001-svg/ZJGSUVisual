...
校园范围：东西 1770m × 南北 910m（v4 按 OSM 实测 bounds）。
相机默认视角：场景中心东南方向 45° 俯视。
```

> **v4 变更（2026-07）**：世界底稿由"手工抽象布局"升级为 **OpenStreetMap 实测**。
> 数据管线 `scripts/fetch-osm.mjs`（Overpass 拉取）→ `scripts/build-world.mjs`（定稿）生成 `campus-zjgsu.json` v4：
> - 21 栋楼全部匹配 OSM 真实平面轮廓（`outline` 多边形，`layoutConfidence: 'osm'`），渲染由盒体升级为轮廓拉伸体；面积/层数仍为官方档案。
> - 新增 `contextBuildings`（核心区内 160 栋配角建筑灰色体量，不可交互，夜景有窗灯）与 `greenery`（绿地多边形）；道路 282 条 / 水系 31 / 校门 3 个均为实测坐标。
> - 抽象布局的系统性方位误差已按实测纠正（如学正街实际位于校区南侧、清风苑实际位于西北）。
> - `distantQuarters` 退役（生活区由真实宿舍楼体量取代）；部分 OSM 未命名主干路按语义补名（`build-world.mjs` 的 `ROAD_NAME_PATCH`）。
> - 渲染范围裁剪至校园核心，邻校要素不渲染。
> - 底图数据 © OpenStreetMap contributors（ODbL），界面左下角常驻标注。
...
环境层：学正街（南界城市路）、乃器路/春华路（校内环路）、学林街（北界）、中央礼仪轴；图书馆景观湖 + 北部景观河；正门（南大门·飞翔门）/北大门/1号门三个实测大门。

配角体量：核心区 160 栋 OSM 建筑（含 46 栋编号学生公寓）灰色渲染，不可下钻，参与天际线与夜景窗灯。
...
> **v4 补充 · 角色数据权限**：`role: visitor | student | admin` 控制视图层（Agent 指令流不受限，路演安全）。
> 学生/访客 = 发起端：可报修、只见「我的报修」进度；受理/办结审批与工单队列不可见；「态势」tab 禁用。
> 管理员 = 受理端（教师/领导）：全校工单队列 + 审批操作；未闭环工单在「态势」tab 红点计数推送。
> 左右侧栏可折叠（`panelCollapsed`，44px 窄条）；预约确认即 Lv3 剖层定位到楼内房间（与报修同机制，房间内点按真实轮廓 2×4 槽位计算，见 `lib/roomPos.ts`）。
